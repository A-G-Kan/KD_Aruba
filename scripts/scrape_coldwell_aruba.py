...
from deduplicate import dedup_within_site, parse_two_sizes, infer_listing_type, restore_user_fields
...
def scrape_detail(page, url):
    try:
        ...
        return beds, baths, building_size, lot_size, status, desc
    ...
    with sync_playwright() as p:
        ...
        while True:
            ...
            for row in rows:
                ...
                beds, baths, building_size, lot_size, status, desc = scrape_detail(page, href)
                time.sleep(0.4)
                ...
                results.append({
                    "id":           slug,
                    "name":         data["name"],
                    "type":         infer_listing_type(data["name"], desc),
                    "image":        data["image"],
                    "area":         data["area"],
